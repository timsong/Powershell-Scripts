...
{
  Write-Host "Windows Server 2008 R2 detected" 
  ...
  {
    Write-Host "Needed cluster features were not found on the machine. Please run the following command to install them:"
    Write-Host "Add-WindowsFeature 'Failover-Clustering', 'RSAT-Clustering'"
    exit 1
  }
}
else 
{
  Write-Host "Windows Server 2012 or above detected"
  ...
  {
    Write-Host "Needed cluster features were not found on the machine. Please run the following command to install them:"
    Write-Host "Add-WindowsFeature 'Failover-Clustering', 'RSAT-Clustering-PowerShell', 'RSAT-Clustering-CmdInterface'"
    exit 1
  }
}
...
# The Split(".") is needed, because user might specify machines using their fully qualified domain name, but we only care about the machine name in the below verification
...
if ($Force)
{
  Write-Host "Forcing cleanup of the specified nodes"

  @($ClusterNodes) | Foreach-Object { Clear-ClusterNode "$_" -Force } 
}
else
{
  Write-Host "Making sure that there is no cluster currently running on the current node"
...
Write-Host "Trying to create a one node cluster on the current machine $env:COMPUTERNAME"

Sleep 5

New-Cluster -Name $ClusterName -NoStorage -Node $LocalMachineName

Write-Host "Verify that cluster is present after creation"
...
Write-Host "Bring offline the cluster name resource"
Sleep 5
Stop-ClusterResource "Cluster Name"

Write-Host "Get all IP addresses associated with cluster group"
...
Write-Host "Found $NumberOfIPs IP addresses"

Write-Host "Bringing all IPs offline"
Sleep 5
$AllClusterGroupIPs | Stop-ClusterResource

Write-Host "Get the first IPv4 resource"
...
Write-Host "Removing all IPs except one IPv4 resource"
Sleep 5
...
Write-Host "Setting the cluster IP address to a link local address"
Sleep 5
...
Write-Host "Adding other nodes to the cluster" 
@($ClusterNodes) | Foreach-Object { 
                       if ([string]::Compare(($_).Split(".")[0],$LocalMachineName, $true) -ne 0) { 
                             Add-ClusterNode "$_" } }

Write-Host "Cluster creation finished !"
exit 0 
